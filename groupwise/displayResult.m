function [out] = displayResult(Fmaps, x0, xsol, xicp, Nsets, Ndims, nICP, isShow, toSave, dataList, nn_method)
...
%         Fmaps{i,j}{1,1}.method{4} = 'GroupBIM';
        ...
        if isfield(Fmaps{i,j}{1,1},'ppMaps')
            Fmaps{i,j}{1,1}.maps{4} = Fmaps{i,j}{1,1}.ppMaps{1};
            Fmaps{i,j}{1,1}.params{4} = 2;
            %Fmaps{i,j}{1,1}.method{3}= 'PairICP';
            Fmaps{i,j}{1,1} = rmfield(Fmaps{i,j}{1,1}, 'ppMaps');
        else
            Fmaps{i,j}{1,1}.maps{4} = Fmaps{i,j}{1,1}.maps{1};
            Fmaps{i,j}{1,1}.params{4} = 2;
            
        end
        
        Fmaps{i,j}{1,1}.maps{5} = Ticp{1}(:,:,i,j);
        Fmaps{i,j}{1,1}.params{5} = 3;
        Fmaps{i,j}{1,1}.maps{6} = Ticp{2}(:,:,i,j);
...
plist_fname = '/benchmark/PairList/VerboseIntraFAUST';
...
for i=1:Nsets
    for j=1:Nsets
        if i~=j
            ...
            elseif ismember(p2(ismember(p1, name1)), name2)
                ...
                Fmaps{i,j} = mapErrorClosestDelta(Fmaps{i,j}{1,1}.mesh1, Fmaps{i,j}{1,1}.basis1,...
                    Fmaps{i,j}{1,1}.mesh2, Fmaps{i,j}{1,1}.basis2, Fmaps{i,j},1:Fmaps{i,j}{1,1}.mesh1.nv,...
                    nn_method);
                ...
                if toSave == 1
                    ...
                    fmap_fname = sprintf('benchmark/Group_d30/useLandmark/nnICP_%s_to_%s.mat', Fmaps{i,j}{1}.mesh1.name, Fmaps{i,j}{1}.mesh2.name);
