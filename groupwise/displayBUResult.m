function [out] = displayBUResult(Fmaps, x0, xsol, xicp, Nsets, Ndims, nICP, isShow, toSave, dataList, nn_method)
% pairwise
T0 = M2T( x0 );
% groupwise
...
plist_fname = '/benchmark/PairList/VerboseBU3DPairs';
...
for i=1:Nsets
    for j=1:Nsets
        if i~=j
            ...
            if isSaveMapsMode == 1
                filename = ['benchmark/BU3DMaps/', name1, '_to_',name2,'.mat'];
                ...
                dest = mapClosestDelta(map, toTest, basis1, basis2, 'ann'); 
                corres = [toTest; dest];
            ...
            elseif sum(l1) > 0 && sum(l2) >0 && sum(l1==l2) > 0
                ...
                Fmaps{i,j}{1,1}.basis1 = Fmaps{i,j}{1,1}.mesh1.confLaplaceBasis(:,1:Ndims);
                Fmaps{i,j}{1,1}.basis2 = Fmaps{i,j}{1,1}.mesh2.confLaplaceBasis(:,1:Ndims);
               ...
               % printMapsErrors(Fmaps{i,j}{1,1}.options, Fmaps{i,j}{1,1}.mesh1, Fmaps{i,j}{1,1}.mesh2, Fmaps{i,j});
                ...
                if toSave == 1
                    ...
                    fmap_fname = sprintf('benchmark/BU3DFmaps/eval_landmark_3/%s_to_%s.mat', Fmaps{i,j}{1}.mesh1.name, Fmaps{i,j}{1}.mesh2.name);
                    save(fmap_fname, 'fmaps','destinations','errors','gwICPs');
                end
