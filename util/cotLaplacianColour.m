...
function [W A] = cotLaplacianColour(X, T, C);
nv = size(X,1);
nf = size(T,1);

mu = 0.01;
% Find orig edge lengths and angles
L1 = normv(X(T(:,2),:)-X(T(:,3),:) + mu * (C(T(:,2),:)-C(T(:,3),:)));
L2 = normv(X(T(:,1),:)-X(T(:,3),:) + mu * (C(T(:,1),:)-C(T(:,3),:)));
L3 = normv(X(T(:,1),:)-X(T(:,2),:) + mu * (C(T(:,1),:)-C(T(:,2),:)));
EL = [L1,L2,L3];
...
% Triangle areas
N = cross(X(T(:,1),:)-X(T(:,2),:) + mu*(C(T(:,1),:)-C(T(:,2),:)), X(T(:,1),:)-X(T(:,3),:)...
    + mu * (C(T(:,1),:) - C(T(:,3),:)));
Ar = normv(N);
